...
subtitle: "AMED3002 Reseach Project 2"
...
```{r Origin data, include=FALSE, cache=TRUE}
...
```{r paper 251 genes, include = FALSE, cache=TRUE}
Genes_copy <- Genes
...
```{r boxplot, fig.cap="Figure 1: Boxplot of Gene Expression of Samples", cache=TRUE}
Expression %>% 
  as.data.frame() %>% 
...
```{r removing duplicate, cache=TRUE}
Expression_cut <- Expression %>% 
  as.data.frame() %>% 
  mutate(GSM1230946=(GSM1230903 + GSM1230904)/2) %>%
  dplyr::select(-GSM1230903) %>%
...
```{r maximum foldchange, cache=TRUE}
...
```{r testset data extraction, echo=TRUE, results=FALSE, cache=TRUE}
...
```{r paper top 20 genes, cache=TRUE} 
...
```{r our top 20 genes, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
```{r table with similar genes for the 251 genes, top 20 genes, cache=TRUE}
...
```{r Classification graphs, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
```{r training scores, fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 2. Support Vector Machine Scores for the Training Set. Left: The Published Paper. Right: Replication Result.", cache=TRUE }
...
```{r heatmap, fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 3. Heatmap based on the Gene Expression of the Training Set. Left: The Published Paper. Right: Replication Result.", cache=TRUE}
...
```{r PCA, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
#PCA genes paper
...
```{r paper PCA , fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 4. Principal Component Analysis of the Training Set and Test Set 1. Left: The Published Paper. Right: Replication Result.", cache=TRUE}
...
```{r svm TS1, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
```{r svm TS1 plots, fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 5. Support Vector Machine Scores for Test Set 1. Left: The Published Paper. Right: Replication Result.", cache=TRUE}
...
```{r svm TS2, echo=TRUE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
```{r svm TS2 plots, fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 6. Support Vector Machine Scores for Test Set 2. Left: The Published Paper. Right: Replication Result.", cache=TRUE}
...
```{r svm TS3-6, roc curve training, cache=TRUE}
#svm TS3
...
```{r combine plots, eval = F, cache=TRUE}
...
```{r roc curves, fig.show = "hold", out.width = "50%", fig.align = "default",fig.cap="Figure 7. ROC plots and AUC values for the Training Set, Test Set 1, Test Set 2, Test Set 3, Test Set 4, Test Set 5, Test Set 6. Left: The Published Paper. Right: Replication Result.", cache=TRUE}
...
```{r performance of classifier table-sensitivity, specificity, accuracy , AUC, cache=TRUE}
...
3.	Bruchfeld J, Correia-Neves M, Källenius G. Tuberculosis and HIV Coinfection. Cold Spring Harb Perspect Med. 2015;5(7):a017871-a.
