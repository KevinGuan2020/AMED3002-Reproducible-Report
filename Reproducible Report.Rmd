...
Expression_43 <- Expression%>% as.data.frame()%>%mutate(GSM1230946=(GSM1230903 + GSM1230904)/2)%>%dplyr::select(-GSM1230903)%>%dplyr::select(-GSM1230904)%>%relocate(GSM1230946,.after=GSM1230902)%>%dplyr::rename(GSM1230903=GSM1230946)
...
for (a in (1:ncol(data)-1)){
  for(i in (1:ncol(data)-a)) {
    i = i + 1
    fc = data[1,a]/data[1,i]
    changes = c(changes, fc)
...
foldchange = data.frame(name = names,
                        change = changes)
```
